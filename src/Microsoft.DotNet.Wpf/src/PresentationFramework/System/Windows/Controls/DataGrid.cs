...
namespace System.Windows.Controls
{
...
#endif
    {
        ...
        protected virtual void OnExecutedCommitEdit(ExecutedRoutedEventArgs e)
        {
            ...
            try
            {
                ...
                if (cell != null)
                {
                    ...
                    {
                        if (!ShimValidateRowCommit(cell.RowOwner))
                        {
        ...
        {
            ShimExecutingCancelEditCommand = true;
            try
            {
                DataGridCell cell = CurrentCellContainer;
                if (cell != null)
            ...
            {
                ShimExecutingCancelEditCommand = false;
            }
...
#if HAS_UNO
                    // On Uno, AddAutoColumns() runs eagerly (no _measureNeverInvoked guard inside it)
                    // so columns may already be present when the DataGrid enters the visual tree.
                    // Skip the second generation to avoid duplicate columns.
                    && !Columns.Any(c => c.IsAutoGenerated)
#endif
                    )
                {
                    AddAutoColumns();
